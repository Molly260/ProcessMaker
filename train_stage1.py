...
from networks.asylora_flux import MultiLayerFeatureExtractor, MultiLayerSupervisionLoss, get_dynamic_layer_weights, get_supervision_weight_schedule
...
class ClassSpecificLoRAMask(torch.nn.Module):
    def __init__(self, num_classes, network_dim):
        super().__init__()
...
class FluxNetworkTrainer(train_network_asylora.NetworkTrainer):
    ...
    def assert_extra_args(self, args, train_dataset_group):
        ...
        if args.use_class_specific_mask:
            if not hasattr(args, 'num_classes'):
                raise ValueError("When using class specific mask, num_classes must be specified")
        ...
        if hasattr(args, 'use_multilayer_supervision') and args.use_multilayer_supervision:
            ...
            if hasattr(args, 'layer_supervision_weights') and args.layer_supervision_weights:
                ...
                if len(weights) == len(target_layers):
                    for i, layer in enumerate(target_layers):
                        layer_weights[f'layer_{layer}'] = weights[i]
                else:
            ...
            logger.info(f"Multi-layer supervision: target_layers={target_layers}, loss_type={loss_type}")
            if layer_weights:
    ...
    def get_noise_pred_and_target(
        ...
        if "custom_attributes" in batch:
            ...
            if len(diff_output_pr_indices) > 0:
                ...
                unwrapped_network.set_multiplier(1.0)  # may be overwritten by "network_multipliers" in the next step
        ...
        return model_pred, target, timesteps, multilayer_supervision_loss, weighting
    ...
    def post_process_loss_with_supervision(self, loss, supervision_loss, args, global_step):
        ...
        if self.use_multilayer_supervision and supervision_loss is not None:
            ...
            if hasattr(args, 'dynamic_layer_weights') and args.dynamic_layer_weights:
                ...
                new_layer_weights = get_dynamic_layer_weights(current_epoch, total_epochs)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of classes for class-specific LoRA mask"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use multi-layer feature supervision to improve generalization"
    )
    parser.add_argument(
        "--target_supervision_layers",
        type=str,
        default="4,9,14,19,29",
        help="Target layers for supervision (comma-separated). Default: 4,9,14,19,29"
    )
    parser.add_argument(
        "--layer_supervision_weights",
        type=str,
        default="0.2,0.3,0.2,0.2,0.1",
        help="Weights for each supervision layer (comma-separated). Default: 0.2,0.3,0.2,0.2,0.1"
    )
    parser.add_argument(
        "--multilayer_supervision_weight",
        type=float,
        default=0.5,
        help="Overall weight for multi-layer supervision loss. Default: 0.5"
    )
    parser.add_argument(
        "--supervision_loss_type",
        type=str,
        default="cosine",
        choices=["cosine", "mse", "combined"],
        help="Type of supervision loss. Default: cosine"
    )
    parser.add_argument(
        "--supervision_schedule",
        type=str,
        default="staged",
        choices=["staged", "linear", "constant"],
        help="Schedule for supervision weight. Default: staged"
    )
    parser.add_argument(
        "--dynamic_layer_weights",
        action="store_true",
        help="Use dynamic layer weights that change during training"
    )
