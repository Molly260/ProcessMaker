...
from library import flux_train_utils_recraft as flux_train_utils
from library.device_utils import init_ipex, clean_memory_on_device

init_ipex()

from library import flux_models, flux_utils, sd3_train_utils, strategy_base, strategy_flux, train_util
...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def cache_text_encoder_outputs_if_needed(
        ...
        if args.cache_text_encoder_outputs:
            ...
            if args.sample_images is not None:
                ...
                with torch.no_grad():
                    for image_item, prompt_dict in zip(sample_images, prompts):
                        ...
                        if prompt not in conditions:
                            if isinstance(image_item, (list, tuple)) and len(image_item) == 2:
                                ...
                                logger.info(f"Cache conditions for single image: {image_path} with prompt: {prompt}")
                                resize_transform = ResizeWithPadding(size=512, fill=255) if args.dataset_frame_num == 4 else ResizeWithPadding(size=352, fill=255)
                                img_transforms = transforms.Compose([
                                    resize_transform,
                                    transforms.ToTensor(),
    ...
    def sample_images(self, accelerator, args, epoch, global_step, device, ae, tokenizer, text_encoder, flux):
        ...
        if self.sample_conditions is not None:
            conditions = {}
            for k, v in self.sample_conditions.items():
                if isinstance(v, list):
                    conditions[k] = [latent.to(accelerator.device) for latent in v]
                else:
                    conditions[k] = v.to(accelerator.device)
    ...
    def get_noise_pred_and_target(
        ...
        if args.dataset_frame_num == 9:
            noise[:, :, :h//3, :w//3] = 0  
            noise[:, :, 2*h//3:h, 2*w//3:w] = 0  
        else:
            noise[:, :, :h//2, :w//2] = 0  
            noise[:, :, h//2:h, :w//2] = 0 
        ...
        if args.dataset_frame_num == 9:
            # 9 frame
            target[:, :, :h//3, :w//3] = latents[:, :, :h//3, :w//3]  
            target[:, :, 2*h//3:h, 2*w//3:w] = latents[:, :, 2*h//3:h, 2*w//3:w] 
        else:
            # 4 frame
            target[:, :, :h//2, :w//2] = latents[:, :, :h//2, :w//2]  
            target[:, :, h//2:h, :w//2] = latents[:, :, h//2:h, :w//2]  
